    ...
    def train(self):
        ...
        for st in range(5):
            ...
            for e in range(1, self.config.train_epoch + 1):
                ...
                #     entity_type = [[e[0] for e in se] for se in entity]
                #     entity_index = [[e[1] for e in se] for se in entity]
                #     # loss = self.model_stu.teach_forward(
                #     #     self.model_tea, x=sentence, x_lens=sentence_len, entity_index=entity_index,
                ...
                #         print('[INFO] {} | Epoch : {}/{} | weak process:{}/{} | train_loss : {}'.format(
                #             datetime.datetime.now(), e, self.config.train_epoch, idx + 1,
    ...
    def eval(self, op='valid'):
        ...
        with t.no_grad():
            for idx, batch in enumerate(valid_iter):
                ...
                #     g_p, g_e = self.model_stu.predict_guided(
                #         x=sentence, x_lens=sentence_len, entity_index=entity_index, entity_type=entity_type,
...
class FewShotLowBT(FewShotLow):
    ...
    def eval(self, op='valid'):
        ...
        print('[INFO] {} | {} result: p = {}, r = {}, f1 = {}'.format(
            datetime.datetime.now(), op, p, r, f1))
        # all_rel_metric = compute_metric(all_elem, all_predict, all_entity)
        # print('[INFO] {} | {} result on all relation: p = {}, r = {}, f1 = {}'.format(
        #     datetime.datetime.now(), op, all_rel_metric[0], all_rel_metric[1], all_rel_metric[2]))
        return p, r, f1
...
def my_job():
    ...
    # model = SelfTrainWorkLL(config)
    # model.load_model('model/5-1/LL/model.pt')
    # model = SelfTrainWorkBT(config)
    # model.load_model('model/5-1/BT-%d/model.pt' % config.trans_type)
    # model.initial()
    # model.train()
    # model.train_weak()
    # model.train_by_entity_replace()
