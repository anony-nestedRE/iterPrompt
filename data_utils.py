...
def prepare_data():
    ...
    # w2id = {w: i for i, w in enumerate(all_words)}
    ...
    # print('abnormal examples = ', error_count)
    # 4
    ...
    # train_e, valid_e = new_examples[:fold[0]*split_len], new_examples[fold[0]*split_len:sum(fold[0:2])*split_len]
...
def analysis_relation():
    ...
    #     entity = [(e[0], tuple(e[1])) for e in item['entity']]
    #     element = [(e[0], tuple(e[1])) for e in item['element']]
    #     layer_num = [0]*len(element)
    #     for i in range(len(entity), len(element)):
    #         layer_num[i] = 1+max(layer_num[element[i][1][0]], layer_num[element[i][1][1]])
    #     all_num.extend(list(filter(lambda x: x > 0, layer_num)))
    # layers = sorted(list(set(all_num)))
    # print('example num = ', len(examples))
    # for l in layers:
    #     print('layer %d = %d' % (l, all_num.count(l)))
...
def generate_ne_entity_replace(train_file, new_file, gt=1, change_prob=0.5):
    ...
    #     rl = len(examples[i]['entity'])
    #     examples[i]['entity'] = sorted(list(set([(e[0], tuple(e[1])) for e in examples[i]['entity']])), key=lambda x: x[1])
...
def generate_ne_entity_replace2(file1, file2, new_file, gt=1, change_prob=0.5):
    ...
    #     rl = len(examples1[i]['entity'])
    #     examples1[i]['entity'] = sorted(list(set([(e[0], tuple(e[1])) for e in examples1[i]['entity']])), key=lambda x: x[1])
    #     examples1[i]['element'] = examples1[i]['entity'] + examples1[i]['element'][rl:]
    # for i in range(len(examples2)):
    #     rl = len(examples2[i]['entity'])
    #     examples2[i]['entity'] = sorted(list(set([(e[0], tuple(e[1])) for e in examples2[i]['entity']])), key=lambda x: x[1])
    #     examples2[i]['element'] = examples2[i]['entity'] + examples2[i]['element'][rl:]
...
if __name__ == '__main__':
    # pass
    # convert_2to1(os.path.join(config.data_dir, 'new-sys-labeled'), os.path.join(config.data_dir, 'new-sys-labeled.json'))
    # prepare_data()
    # generate_type()
    # test_data_loader()
    # analysis_relation()
    # generate_k_fold_data2(5)
    # from utils import save_config
    # save_config(config, 'config.txt')
    # all_entity = get_all_entity(os.path.join(config.data_dir, 'train.json'))
    # json.dump(all_entity, open(os.path.join(config.data_dir, 'entity.json'), 'w', encoding='utf-8'), ensure_ascii=False)
    # generate_ne_entity_replace(os.path.join(config.data_dir, 'train.json'), os.path.join(config.data_dir, 'g-train.json'))
    # get_error_data()
    # generate_merge_example(os.path.join(config.data_dir, 'train.json'), os.path.join(config.data_dir, 'm-train.json'))
    # convert_1to2(os.path.join(config.data_dir, 'abnormal.json'), os.path.join(config.data_dir, 'new-sys'))
    # convert_2to1(os.path.join(config.data_dir, 'new-sys-labeled'), os.path.join(config.data_dir, 'new-train.json'))
    # generate_raw_weak_data(config.data_dir, ratio=0.3)
    # generate_n_way_k_shot(config.data_dir, 5, 1)
    # generate_n_way_k_shot(config.data_dir, 5, 5)
    # generate_n_way_k_shot(config.data_dir, 5, 10)
    # generate_shot_weak_data(config.data_dir, k=10)
    # generate_n_way_k_shot_low(config.data_dir, 5, 1)
    # generate_n_way_k_shot_low(config.data_dir, 5, 5)
    # generate_n_way_k_shot_low(config.data_dir, 5, 10)
    # generate_n_way_k_shot2(config.data_dir, 5, 1)
    # generate_n_way_k_shot2(config.data_dir, 5, 5)
    # generate_n_way_k_shot2(config.data_dir, 5, 10)
