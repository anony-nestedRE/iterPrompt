...
def merge_result(model_dir, k_fold=None, res_file='metric.xlsx'):
    ...
    if k_fold is None:
        ...
        # merged_file = os.path.join(model_dir, 'metric.xlsx')
    ...
    for i in range(b):
        for j in range(i+1, b):
            try:
                ...
                pass
                # print({'entity': examples['entity'][i], 'element': examples['element'][i]},
                #       {'entity': examples['entity'][j], 'element': examples['element'][j]})
            # sim[i][j] = compute_similarity_structure(
            #     {'entity': examples['entity'][i], 'element': examples['element'][i]},
            #     {'entity': examples['entity'][j], 'element': examples['element'][j]}
...
def t_compute_similarity_structure(train_file):
    ...
    #     sim = compute_similarity_structure(examples[0], examples[i])
    #     if sim > m_sim:
    #         m_idx = i
    #         m_sim = sim
    # print(examples[0]['sentence'], examples[m_idx]['sentence'], '\nsim = %.5f' % m_sim)
    le = len(examples)
    m_sim = [0] * le
    idx = [-1] * le
    for i in range(le):
    ...
    for i, j in enumerate(idx):
        # pair_writer.write(
        #     json.dumps(examples[i], ensure_ascii=False)+','+
        #     json.dumps(examples[j], ensure_ascii=False)+','+'%.5f' % m_sim[i] + '\n')
...
def analysis_few_nk_res(file):
    ...
    #     for m in model_list:
    #         for lr in lr_list:
    #             writer.write(','.join(['0', k, m, lr, res[cnt]])+'\n')
    #             cnt += 1
    # for cw in cw_list:
    #     for k in k_list:
    #         for m in ['BT', 'BTLow']:
    #             for lr in lr_list:
    #                 writer.write(','.join([cw, k, m, lr, res[cnt]])+'\n')
    #                 cnt += 1
    writer.close()
...
if __name__ == '__main__':
    # merge_result('model', 5)
    # merge_result('model', 5, 'guide.xlsx')
    # from config import Config
    # t_config = Config()
    # test_compute_similarity_structure(os.path.join(t_config.data_dir, '%s.json' % t_config.train_prefix))
    # deal_flat()
    # analysis_train()
    log_dir = 'E:\master\python\RE\\relation\PLM\\tmp-code'
    # analysis_few_nk_res(os.path.join(log_dir, 'few-c-all-22-13.out'))
    mn = 'k-BT-c5-1211'
    ...
    # get_layer_res(os.path.join(log_dir, 'k-BT-c-r111.out'))
